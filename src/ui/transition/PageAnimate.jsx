...
function PageAnimate({ children }) {
  ...
  const handleTransitionEnd = () => {
    if(nextPath){
      if(Array.isArray(prevPage) && prevPage.length == 0)
        dispatch(setPrevPage(currentPath))
      dispatch(setPrevPage(nextPath))
  ...
  useEffect(() => {
    if(location.pathname != currentPath){
      ...
      if((_cur[1] != _new[1] || _new[1] == 'register' || (!isNaN(_new[2]) && _new[2] != '' && _cur[1] != 'offers') || ['chat', 'settings'].includes(_cur[1]) || _cur[2] == 'new' || _new[2] == 'new')){
