...
const Header = () => {
  ...
  const logout = async (e) => {
    e.preventDefault();
    if (await logoutUser()) dispatch(updateUser({}));
  };
  ...
  useEffect(() => {
    ...
    if (
      user?.id &&
      ["/register/user/", "/register/company"].some(
      ...
      !user?.id &&
      ["/dashboard/", "/dashboard/notifications/"].some(
        (url) => url == location.pathname
      )
    )
      navigate("/login/");
    ...
    <header className={header}>
      ...
      {!["intro", "dashboard"].some((s) => s == header) && (
        <div className="navbar-item go-back">
          ...
          {addLink && (
            <Link className="plus" to={addLink}>
      ...
      {!["intro", "login", "register", "settings", "map"].some(
        (s) => s == header
      ) && (
        <>
          <div className="navbar-item icons">
            {location.pathname != "/dashboard/" ? (
              <div className="navbar-item" style={{ width: "auto" }}>
