...
import { Notifications, Offers, OfferNew, Chats, Chat, Followers, Bookmarks, Settings, ProfileSettings, Password, Terms, About } from "@components/modules"
  ...
  useEffect(() => {
    ...
    <Routes>
      ...
      {(user.role == 'user') && <>
        ...
        <Route exact path="/shelter/:id" element={<Profile />} />
        ...
        <Route exact path="/social/:id" element={<Profile />} />
      ...
      {(user.role == 'vendor' || user.role == 'social' || user.role == 'ngo') && <>
       ...
       {(user.role == 'ngo') && <>
        <Route exact path="/profile/pets" element={<Profile type="pets" />} />
      </>}
